...
` + reference('dsfr-data-beacon'),
  ...
  // ---------------------------------------------------------------------------
  // Grammaires d'attributs et voies natives (#657)
  // ---------------------------------------------------------------------------
...
marche pas ».`,
  ...
  dsfrLayout: {
    ...
    content: `## Mise en page DSFR d'un tableau de bord
...
7. Vérifier la page à 320 px avant de la livrer : c'est la résolution de maquette mobile du
  ...
  // ---------------------------------------------------------------------------
  // Gabarits de page : 4 familles relevees sur le banc d'essai open-data-viz
  // ---------------------------------------------------------------------------

  pagePatterns: {
    id: 'pagePatterns',
    name: 'Gabarits de page',
    description:
    ...
    content: `## Gabarits de page : quelle page pour quelle question
...
export function getRelevantSkills(message: string, currentSource: Source | null): Skill[] {
  ...
  // Une page complete se compose AUSSI : compositionPatterns dit quoi brancher,
  // dsfrLayout dit ou le poser (grille DSFR, espacements, gabarits). Les deux
  // vont ensemble des qu'il s'agit d'une page entiere et non d'un widget isole.
  if (
    lowerMsg.match(
      /dashboard|tableau de bord|mise en page|colonnage|grille|gabarit|align|espacement|responsive|mobile/
    ) &&
    !relevant.find((s) => s.id === 'dsfrLayout')
  ) {
    relevant.push(SKILLS.dsfrLayout);
  }
