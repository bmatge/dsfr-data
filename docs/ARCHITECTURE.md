...
dsfr-data-source  ──[fetch via adapter]──[paginate]──[cache]──► donnees brutes
  ...
  … ou, avec scopes="scores:code:q" (#891), le repeteur EMET les ids lui-meme :

  dsfr-data-source (scores) ──► dsfr-data-repeat ──┬──► q-001 (lignes de la cle) ──► dsfr-data-chart
                                                   ├──► q-002 … ──► dsfr-data-chart
                                                   └──► … (une partition, 119 ids)
...
- **dsfr-data-repeat** (#890, ADR-135) est le seul noeud de **structure** : il ne rend pas la donnee, il fabrique des instances — pour chaque ligne de `source`, le `<template>` enfant est clone en DOM (`utils/template-clone.ts`) et ses placeholders resolus noeud par noeud avec le `renderTemplate` partage (`escape: false`) ; les composants du gabarit sont rehausses avec leurs attributs deja interpoles. `key-field` donne l'identite : a une re-emission, une cle qui subsiste garde ses noeuds (mise a jour en place), jamais de deconnexion-recreation sous le meme id. Rendu transparent (aucun `role`, aucun compteur). Regle d'usage : `display` quand la ligne est du contenu, `repeat` quand la ligne est un pipeline. `SourceSubscriberMixin` — mais **il emet aussi** depuis le lot 2 (`scopes`, #891) : il s'abonne en plus a chaque source declaree par `scopes`, la partitionne UNE fois par champ (`Map<cle, lignes>`) et `dispatchDataLoaded('<alias>-<cle>', lignes)` par ligne repetee, avec la meta reduite au `total` du scope (`truncated` et `serverSide` ne se relaient pas : aucun serveur ne pagine un id fabrique). Une cle sans lignes emet un **tableau vide**. Les etats `loading` / `error` / `idle` de la source scopee sont relayes tels quels sur chaque id scope, et rejoues pour une ligne qui vient de naitre (un evenement passe ne se rattrape pas au cache, contrairement aux lignes). Purge (`clearDataCache` + `clearDataMeta`) quand une ligne disparait et a la deconnexion. Ce qu'il ne fait pas : **aucune delegation** derriere un id scope — meme regle #765 que N queries sur une source partagee, la partition est cliente. `lazy` retient les elements `dsfr-data-*` du clone hors du document (un commentaire a leur place) jusqu'a l'entree de la ligne dans une marge de 200 px : jamais connectes, donc jamais abonnes ; le reste du gabarit est rendu d'emblee.
...
- `tests/skills/skills-reference.test.ts` controle la chaine **maillon par maillon** : le manifeste decrit exactement les attributs qui existent au runtime (introspection Lit `elementProperties`), le module genere commite est le rendu exact du manifeste, et chaque skill embarque sa section generee tout en gardant un guide. **Un attribut ajoute sans `npm run build:skills` fait echouer le test.**
- `tests/skills/skills.test.ts` ne garde plus que les alignements portant sur le texte redige a la main (types de graphiques, operateurs, agregations, palettes).
...
- **Empilement.** Le panneau a un `z-index` de 770 : SOUS le tiroir Diagnostic (780), SOUS la barre d'actions épinglée (775) et l'en-tête (776), dont les menus (« Plus d'actions », Mon espace, sélecteur d'app) sont enfermés dans leur contexte d'empilement — à 700, « Plus d'actions » s'ouvrait sous le volet et `#diagnostic-btn` devenait inatteignable. Avec un tiroir, quand le Diagnostic s'ouvre, l'assistant se réduit (`diagnostic-toggle`) ; ouvrir l'assistant ne ferme jamais le Diagnostic. Intégré, le Diagnostic est un onglet du panneau (§3.8). En-tête du panneau sur une ligne (titre, onglets, actions ; sous-titre en `fr-sr-only`) ; « Me montrer un réglage : Dire · Guider » sous le champ de saisie, **Guider par défaut** (`getReperageMode()`, arbitrage du 2026-09-23). Le bas du panneau s'arrête à `calc(--app-action-bar-fixed-h + --app-diagnostic-h)` : il ne recouvre jamais le rail ni la barre d'actions fixe. Le haut commence au bas de la barre d'actions, mesuré par `app-action-bar` et publié dans `--app-action-bar-bas` (ResizeObserver, défilement, redimensionnement) : la barre et sa primaire restent cliquables volet ouvert. Sans `app-action-bar` (Sources), `mountAssistant` publie la même variable depuis la rangée d'actions du bouton (`closest('[data-zone]')`) ; sans rien de tout cela, le haut suit `--app-header-h`, uniquement sous `PINNED`. Une languette (`.assistant-lanceur`, `fr-icon-sparkling-2-line`) collée au bord droit, à mi-hauteur de la zone libre, ouvre aussi le volet : elle est masquée volet ouvert, porte la même pastille que `#assistant-btn` (conservé, repère `<app>.actions.assistant`), et reprend le focus à la réduction si c'est elle qui a ouvert. Sous 35.98em, elle devient une pastille ronde en bas à droite, au-dessus de `--app-action-bar-fixed-h` et `--app-diagnostic-h`. Sous 35.98em, le panneau devient une feuille plein écran, mais toujours au-dessus du mobilier bas. Sous 48em, la raison de désactivation de la primaire est masquée à l'écran tant qu'il est ouvert. Tout cela est testé dans `tests/apps/app-ui/chrome-mobile.test.ts`.
...
- **Repères hors registre (#1105).** Une app peut montrer des repères absents du registre (Playground : les lignes du code, `playground.ligne.<n>.<balise>[.<attribut>]`). Trois options de `mountAssistant` : `correspondanceHorsRegistre(question)`, consultée AVANT le modèle quand le registre ne trouve rien ou seulement un repère de `reperesEnglobants` (l'éditeur de code, trop vague quand une ligne répond) ; `libelleHorsRegistre(id)`, le filtre : un id hors registre, du modèle comme de la correspondance, n'est accepté qu'après `estIdRepere()` ET si l'app rend un libellé (la ligne existe dans le code courant, y porte cette balise et l'attribut nommé) ; `montrerHorsRegistre(id)`, qui le montre. Côté Albert, `ProfilAssistant.contexte(etat)` ajoute un bloc au prompt (jetons d'URL masqués) et ses ids à l'enum de `montrer` seulement (le plan pas à pas suit le registre). Le Playground y met un plan compact du code (60 balises au plus, valeurs raccourcies, `headers`/clés masqués) : `apps/playground/src/assistant/correspondance-code.ts`.
...
- **Deux chemins d'import, un seul aplatissement** — `packages/shared/src/providers/flatten.ts`. Un jeu de donnees entre dans l'app par deux routes independantes : le **chemin composant** (adapters de `packages/core`, ex. `insee-adapter.ts:234`) et le **chemin connexion** (`apps/sources/src/connections/api-explorer.ts:267`). Les providers dont les enregistrements sont imbriques (INSEE en `attributes`/`dimensions`/`measures`, Grist sous `fields`) doivent produire **exactement les memes noms de colonnes** par les deux routes, sinon un graphique construit depuis une connexion casse quand la meme source est rechargee par un composant. La strategie est donc declaree une fois dans `ProviderConfig.response` (`flattenRecord`, sinon `nestedDataKey`) et appliquee par `flattenProviderRecords()`, partage par les deux. **Piege historique** : `requiresFlatten` et `nestedDataKey` ont existe pendant des mois **sans aucun consommateur** — les observations INSEE arrivaient en `[object Object]` dans les tables du chemin connexion (#586). Ajouter un provider imbrique sans renseigner l'un des deux champs reproduit le bug en silence.
  **Troisieme route, le mode URL de `dsfr-data-source` (#1136)** : `detectProvider(url)` une fois en tete de `_fetchViaUrl`, puis `flattenProviderRecords(lignes, provider.response)` — le fournisseur est reconnu au CHEMIN (`/api/docs/…/tables/…` pour Grist, `melodi/data/…` pour INSEE), donc aussi derriere un proxy. L'ancien `flattenGristEnvelope` (test de forme dans le composant, #482) est supprime. Ecart connu, anterieur : `GristAdapter._flattenRecords` retire l'`id` Grist, `flattenNestedKey` (connexion, mode URL) le garde. La convention `paginate` du mode URL (`page`/`page_size` en requete, `data` + `meta.{page,page_size,total}` en reponse) est declaree dans `GENERIC_CONFIG.pagination` (`params`, `serverMeta`) et lue par la source.
...
- **L'empilement de la carte se joue sur l'HÔTE, jamais sur le conteneur Leaflet** — `packages/core/src/components/dsfr-data-map.ts` (règles `dsfr-data-map` et `.dsfr-data-map__container` de la feuille injectée) et `dsfr-data-map-popup.ts` (`.dsfr-data-map-popup__panel`). Le mobilier flottant de la carte — sélecteur de fond (#744), bouton de plein écran (#780), bandeau max-items — est posé **en frère** du conteneur Leaflet, pour être atteint au clavier avant la carte ; le volet latéral d'un popup en `mode="panel-*"` est posé **dedans**, pour ne pas recouvrir les encarts. Les deux ne concourent donc dans le même contexte d'empilement que si le conteneur n'en ouvre pas : Leaflet lui pose `position: relative` en style en ligne, et un `z-index: 0` suffisait à sceller tout son sous-arbre sous le mobilier — le volet, pourtant à 1001, passait sous le sélecteur à 1000, dont il masquait le titre et les premières lignes. **Aucun z-index de descendant ne rattrape cela** : la frontière est portée par l'hôte (`isolation: isolate`), qui garde en plus le mobilier à 1000 de recouvrir l'en-tête de la page hôte — la Carto a dû se construire la sienne (`.carto-canvas`, `tests/apps/builder-carto/stacking.test.ts`). **Corollaire, mesuré** : le mobilier recouvre nécessairement les bulles Leaflet, et aucune valeur ne l'en empêche — `.leaflet-map-pane` est lui-même un contexte d'empilement (`position: absolute` + `z-index: 400` + `transform`), ses volets internes (popups à 700) n'ordonnent qu'entre eux ; au-dessus de 400 on couvre tout, en dessous on passe sous les tuiles. Ce qui doit couvrir le mobilier se pose donc hors du conteneur, ou l'accepte. Garde-fous : `tests/map-empilement.test.ts` lit la **structure** (aucun z-index sur le conteneur, `isolation` sur l'hôte) — l'ordre des nombres seul était déjà juste et n'a rien vu — et `e2e/layout-map.spec.ts` mesure l'empilement réel par `elementFromPoint`.
...
- **Composition par échelle de la Carto : la couche agrégée a SA source, sans `limit` ni `select`** (#1021) — `apps/builder-carto/src/composition-echelle.ts`, `ui/code-generator.ts` (`agregatTags`, `layerOutputId`). La choroplèthe compte par code de département/région avec une `dsfr-data-query` qui doit rester **seule lectrice** de sa source, sinon le regroupement n'est plus délégué et la couche de points recevrait des lignes agrégées (#765) ; d'où une copie de la source, jamais partagée, et un compagnon a11y qui lit les points ou le comptage, jamais la source agrégée. Pas de `limit` (le comptage porte sur tout le jeu) ni de `select` (Tabular refuse `columns` à côté d'un agrégateur). Le fond joint est celui du paquet (`geo/*.json`, par le CDN npm : une instance auto-hébergée ne sert pas `geo/`), et la jointure compare la clé **brute** : la détection du champ refuse les codes numériques (`1` pour `01`). Le déclencheur est le `total` de `dsfr-data-map-layer-render` rapporté par l'aperçu (#1020), jamais persisté. Contrôle : `affichages/carte-composition-echelle`.
  L'assistant la propose (lot 2) par le constat **`carte/jeu-tronque`** (`constats-carto.ts`) : la source que lit une couche porte `meta.truncated` — le cas ordinaire d'un gros jeu depuis que le builder aligne `limit` sur `max-items` (#1020), où `carte/tronque-max-items` ne parle pas puisque la couche dessine tout ce qu'elle reçoit. Il remplace `pipeline/tronque` sur cette source et porte des **`remedes`** (champ optionnel de `Constat` : `{ libelle, repere }`, repères identiques à `reperes` et dans le même ordre) : composer par échelle (pas sur une couche de zones ni déjà composée, `min-zoom`), filtrer en amont, relever le plafond (seulement quand la coupe est le `limit` d'une source). Pas de « zone visible » : sur Tabular, `bbox` reste un filtre client tant que #1023 n'est pas implémenté. `app-assistant` rend un bouton par remède à la place de « Me montrer » ; le Diagnostic, lui, montre le premier repère, l'encart. Le bouton de l'encart porte le prérequis `composition-proposee` (champ territoire détecté, couche ni agrégée ni composée), levé par la liste des couches.
