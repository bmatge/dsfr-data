...
| Toutes (panneau Assistant) | **Construire pour moi dans le Studio** | bouton du panneau | passation vers le Studio IA : le diagnostic de l'app est déposé (`transmettreDiagnostic`) puis posé, sans être envoyé, dans le champ du Studio (`recupererDiagnostic()`) (#1016). Depuis #1132, même passation que « Envoyer au Studio IA » dans le Playground (code et source compris) |
| Playground | **Envoyer au Studio IA** | tertiaire, `fr-icon-sparkling-2-line`, repère `playground.actions.studio` | reprendre le code dans le Studio IA (#1132). Pas une entrée « Ouvrir dans » : le code n'y est pas ouvert tel quel, il y est **reconstruit** en blocs. Même voie que « Construire pour moi dans le Studio » (`construireDansLeStudio`) : diagnostic déposé, plus le code et l'adresse publique de sa source (`transmettrePassationStudio`) ; le Studio charge la source par le chemin de `charger_source_url` (une source à clé n'est pas transmise, et il le dit), pose la consigne de reconstruction dans son champ **sans l'envoyer**, et offre « Retour au Playground » |
...
| Playground | Exécuter | Copier le code | Ajouter aux favoris · Ouvrir dans le Pipeline · Envoyer au Studio IA · Exporter en PNG · Exporter en JPG · Ajouter des dépendances · **Diagnostic** · **Assistant** · Réinitialiser |
