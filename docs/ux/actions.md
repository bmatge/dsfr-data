...
| Toutes (panneau Assistant) | **Construire pour moi dans le Studio** | bouton du panneau | passation vers le Studio IA : le diagnostic de l'app est déposé (`transmettreDiagnostic`) puis posé, sans être envoyé, dans le champ du Studio (`recupererDiagnostic()`) (#1016). Depuis #1132, même passation que « Ouvrir dans le Studio IA » dans le Playground (code et source compris) |
...
| Playground | Exécuter | Copier le code | Ajouter aux favoris · Ouvrir dans le Pipeline · Ouvrir dans le Studio IA · Exporter en PNG · Exporter en JPG · Ajouter des dépendances · **Diagnostic** · **Assistant** · Réinitialiser |
