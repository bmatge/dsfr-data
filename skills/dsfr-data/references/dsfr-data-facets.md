...
|---|---|---|---|
| `cols` | `string` | `""` (vide) | Colonnage DSFR des facettes : "6" (global) ou "field:4 \| field2:6" (par facette), en colonnes de la grille de 12 — une LARGEUR, pas un nombre de facettes par ligne. Appliqué à partir de 768 px ; en dessous, chaque facette occupe toute la ligne (`fr-col-12 fr-col-md-N`, #788). Sans `cols`, grille automatique qui se replie seule. |
...
| `fields` | `string` | `""` (vide) | Champs à exposer comme facettes (virgule-séparés). Vide = auto-détection sur les données chargées ; en `server-facets`, vide = découverte des facettes déclarées par le jeu de données, pour les adaptateurs qui savent les découvrir (#680 — métadonnées du jeu, colonnes à choix ; détail par fournisseur dans la table des capacités d'ARCHITECTURE) |
