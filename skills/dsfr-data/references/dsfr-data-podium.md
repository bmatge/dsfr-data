...
| `rank` | `string` | `'number'` | Rendu du rang : `number` (chiffre, défaut), `medal` (pastille de la couleur de l'item) ou `none` (masqué — l'ordre et la barre suffisent). En `medal`, la couleur d'encre du chiffre est choisie par calcul de luminance relative : la rampe s'éclaircit, et du blanc sur le 5e ton serait illisible. Le chiffre reste `aria-hidden` : l'ordre est porté par la position dans la liste, le chiffre n'en est qu'un rappel. |
